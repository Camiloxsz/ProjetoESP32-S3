...
void setup() {
    ...
    // CORREÇÃO CRÍTICA: Aguarda a porta serial USB estabilizar no PC para evitar travamentos
    unsigned long timeout = millis();
    while (!Serial && (millis() - timeout < 5000)) {
        delay(10);
    }
    delay(2000); // Delay de segurança extra
...
void loop() {
    ...
    // Reordenação da Janela Deslizante (Filtro de Falso Positivo)
    ...
    // --- LÓGICA DE DECISÃO E CLASSIFICAÇÃO VIA FINGERPRINTING ---
    ...
    // --- SAÍDA SERIAL FORMATADA COM UNIDADES ---
    ...
    if (alertaReal && (millis() - ultimoAlertaTelegram > INTERVALO_MSG)) {
        ...
        if (statusRede.indexOf("ASSINATURA") != -1) {
